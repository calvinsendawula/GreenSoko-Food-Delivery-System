...
if (isset($_POST['uploadfooditem'])) {
    ...
    $restaurantImage = $_FILES['restaurantImage']['tmp_name'];
    $restaurantPrice = $_POST['restaurantPrice'];

    $filePath = "images/";

    $originalFileName = $_FILES['restaurantImage']['name'];
    if (move_uploaded_file($restaurantImage, $filePath.$originalFileName)) {
        $sql = "UPDATE `tbl_restaurant` SET `restaurantName`= '$restaurantName', `restaurantImage` = '$originalFileName' WHERE `restaurantID` = '$id'";
...
if(isset($_GET['id'])) {
    ...
    $db = mysqli_connect('localhost', 'root', '', 'green_soko');
    $sql = "SELECT * FROM `tbl_restaurant` WHERE `restaurantID` = $id ";
    $result = mysqli_query($db, $sql);
        ...
        if (mysqli_num_rows($result) >0) {
            while( $row = mysqli_fetch_assoc($result)) {
                $id = $row['restaurantID'] ;
                $foodName = $row['restaurantName'] ;
                $foodImage = $row['restaurantImage'] ;
            }
...
<body>
  <div class="signupFrm">
      <form action="editRestaurant.php" method="POST" enctype="multipart/form-data" class="form">
...
<div class="inputContainer">
    <input type="text" name="restaurantID" class="input" hidden value="<?php echo $id ?>" required>
    <input type="text" name="restaurantName" class="input" placeholder="Enter the food name" value="<?php echo $foodName ?>" required>
